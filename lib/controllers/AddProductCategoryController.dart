...
class AddProductCategoryController extends GetxController {
  ...
  void AddProductCategory(context) async {
    ...
    try {
      ...
      }, ApiUrl.productCategoryAdd, allowHeader: true);
      loadingIndicator.hide(context);
  ...
  void UpdateProductCategory(context, String productCategoryId) async {
    var loadingIndicator = LoadingProgressDialog();
    try {
      if (networkManager.connectionType == 0) {
        loadingIndicator.hide(context);
      ...
      var response = await Repository.put({
        ...
        "image_id": uploadImageId.value.toString(),
        "description": descCtr.text.toLowerCase().trim(),
      }, '${ApiUrl.editProdcutCategory}/$productCategoryId', allowHeader: true);
      loadingIndicator.hide(context);
      var data = jsonDecode(response.body);
      logcat("RESPOSNE", data);
      if (response.statusCode == 200) {
